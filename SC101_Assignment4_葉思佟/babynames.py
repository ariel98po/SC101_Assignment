...
def add_data_for_name(name_data, year, rank, name):
    ...
    if name in name_data:
        # If the name is ranked differently for both boys and girls in the same year,
        # only keep the one with higher rank (which is lower number)
...
def add_file(name_data, filename):
    ...
    with open(filename, 'r') as f:
        year = f.readline() # The first line is the year
        year = year.strip()
        for line in f:
            baby_name_rank = line.split(',')  # The words are separated by comma
            rank = baby_name_rank[0]
            rank = rank.strip()
            boy_name = baby_name_rank[1]
            boy_name = boy_name.strip()
            girl_name = baby_name_rank[2]
